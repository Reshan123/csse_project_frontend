...
const MedicalRecordForm: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.contactNumber && !phoneRegex.test(formData.contactNumber)) {
      newErrors.contactNumber = 'Invalid contact number format (should be 10 digits)';
    }
    if (formData.emergencyContactNumber && !phoneRegex.test(formData.emergencyContactNumber)) {
      newErrors.emergencyContactNumber = 'Invalid emergency contact number format (should be 10 digits)';
    }
