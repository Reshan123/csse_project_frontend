...
const Home = () => {
  ...
  const { appointments, loading }: { appointments: Array<{ id: string, appointmentDate: string, docName: string, department: string }>, loading: boolean } = appointmentsContext;
          ...
          <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
            ...
            <div className="space-y-4">
              {upcomingAppointments.length > 0 ? (
                upcomingAppointments.map((appointment: { id: string, appointmentDate: string, docName: string, department: string }) => (
                  <div key={appointment.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-teal-50 p-4 rounded-lg">
                    ...
                    <div className="flex items-center justify-between w-full sm:w-auto">
                      <div className="text-left sm:text-right">
                        ...
                        <p className="text-sm text-gray-600">{new Date(appointment.appointmentDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
